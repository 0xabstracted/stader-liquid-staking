...
use error::StaderLiquidStakingError;
...
declare_id!("SdsLUAYNXCpwdkJ7skmjaF8PxiqoUmNmAQfN2D2Jwmb");


#[cfg(not(feature = "no-entrypoint"))]
solana_security_txt::security_txt! {
    name: "stader-liquid-staking",
    project_url: "https://www.staderlabs.com/",
    contacts: "https://x.com/staderlabs",
    policy: "https://stader.gitbook.io/stader",
    source_code: "https://github.com/Stader-Finance/liquid-staking-program",
    preferred_languages: "en",
    auditors: "https://www.staderlabs.com/docs-v1/category/auditors"
}

#[program]
pub mod stader_liquid_staking {
    ...
    pub fn liquid_unstake(ctx: Context<LiquidUnstake>, stader_sol_amount: u64) -> Result<()> {
        // check_context(&ctx)?;
        ctx.accounts.process(stader_sol_amount)
    }
    ...
    pub fn config_stader(
        ctx: Context<ConfigStader>,
        params: ConfigStaderParams,
    ) -> Result<()> {
        // check_context(&ctx)?;
        ctx.accounts.process(params)
    }

    pub fn update_stader_sol_token_metadata(
        ctx: Context<UpdateStaderSolTokenMetadata>,
        name: String,
        symbol: String,
        uri: String,
    ) -> Result<()> {
    ...
    // * update (compute stake-account rewards & update staderSOL price)
    //-------------------------------------------------------------------------------------

    pub fn order_unstake(ctx: Context<OrderUnstake>, stader_sol_amount: u64) -> Result<()> {
        ctx.accounts.process(stader_sol_amount)
    }
    ...
    pub fn withdraw_stake_account(
        ...
        // check_context(&ctx)?;
        ctx.accounts
            .process(stake_index, validator_index, stader_sol_amount, beneficiary)
    }
