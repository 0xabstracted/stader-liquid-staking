use crate::StaderLiquidStakingError;
...
pub fn check_stake_amount_and_validator(
    ...
    let currently_staked = if let Some(delegation) = stake_state.delegation() {
        require_keys_eq!(
            ...
            *validator_vote_pubkey,
            StaderLiquidStakingError::WrongValidatorAccountOrIndex
        );
        delegation.stake
    } else {
        return err!(StaderLiquidStakingError::StakeNotDelegated);
    };
    ...
    if currently_staked != expected_stake_amount {
        ...
        return err!(StaderLiquidStakingError::StakeAccountNotUpdatedYet);
    }
    Ok(())
}
...
pub fn check_token_source_account<'info>(
    ...
    if source_account.delegate.contains(authority) {
        ...
        // delegated_amount & delegate must be set on the user's staderSOL account before calling OrderUnstake
        require_lte!(
            token_amount,
        ...
        return err!(StaderLiquidStakingError::WrongTokenOwnerOrDelegate)
