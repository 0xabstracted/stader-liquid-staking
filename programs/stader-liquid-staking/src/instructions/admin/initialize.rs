...
pub struct Initialize<'info> {
    ...
    pub stader_sol_leg: Box<Account<'info, TokenAccount>>,

    #[account(token::mint = stader_sol_mint)]
    pub treasury_stader_sol_account: Box<Account<'info, TokenAccount>>,
...
impl<'info> Initialize<'info> {
    ...
    fn check_stader_sol_mint(&mut self) -> Result<u8> {
        let (authority_address, authority_bump_seed) =
            State::find_stader_sol_mint_authority(self.state_address());

        check_mint_authority(&self.stader_sol_mint, &authority_address, "stader_sol_mint")?;
        check_mint_empty(&self.stader_sol_mint, "stader_sol_mint")?;
        check_freeze_authority(&self.stader_sol_mint, "stader_sol_mint")?;
    ...
    pub fn process(
        ...
        require_lte!(
            ...
            StaderLiquidStakingError::RewardsFeeIsTooHigh
        );
        ...
        self.state.set_inner(State {
            ...
            stader_sol_supply: 0,
            stader_sol_price: State::PRICE_DENOMINATOR,
        ...
        emit!(InitializeEvent {
            ...
            stader_sol_mint: self.stader_sol_mint.key(),
            operational_sol_account: self.operational_sol_account.key(),
            lp_mint: self.lp_mint.key(),
            lp_stader_sol_leg: self.stader_sol_leg.key(),
            treasury_stader_sol_account: self.treasury_stader_sol_account.key(),
        });
    ...
    pub fn check_lp_mint(&self) -> Result<u8> {
        require_keys_neq!(self.lp_mint.key(), self.stader_sol_mint.key(),);
        let (authority_address, authority_bump_seed) =
    ...
    pub fn check_stader_sol_leg(&self) -> Result<u8> {
        check_token_mint(
            &self.stader_sol_leg,
            &self.stader_sol_mint.key(),
            "liq_stader_sol",
        )?;
        let (stader_sol_authority, stader_sol_authority_bump_seed) =
            LiqPool::find_stader_sol_leg_authority(self.state_address());
        check_token_owner(&self.stader_sol_leg, &stader_sol_authority, "liq_stader_sol_leg")?;
        Ok(stader_sol_authority_bump_seed)
    }
    ...
    pub fn initialize_liq_pool(
        ...
        let stader_sol_leg_authority_bump_seed = self.check_stader_sol_leg()?;
        let liq_pool = LiqPool {
            lp_mint: *self.lp_mint.to_account_info().key,
            lp_mint_authority_bump_seed,
            sol_leg_bump_seed,
            stader_sol_leg_authority_bump_seed,
            stader_sol_leg: *self.stader_sol_leg.to_account_info().key,
            lp_liquidity_target,
            lp_max_fee,
            lp_min_fee,
