...
use crate::{
    error::StaderLiquidStakingError, events::admin::ReallocStakeListEvent, state::stake_system::StakeList,
    State,
};
...
impl<'info> ReallocStakeList<'info> {
    pub fn process(&mut self, capacity: u32) -> Result<()> {
        require_gte!(
            ...
            StaderLiquidStakingError::ShrinkingListWithDeletingContents
        );
