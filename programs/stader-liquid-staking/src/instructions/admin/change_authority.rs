...
pub struct ChangeAuthorityData {
    ...
    pub treasury_stader_sol_account: Option<Pubkey>,
...
impl<'info> ChangeAuthority<'info> {
    pub fn process(&mut self, data: ChangeAuthorityData) -> Result<()> {
        ...
        let treasury_stader_sol_account_change =
            if let Some(treasury_stader_sol_account) = data.treasury_stader_sol_account {
                let old = self.state.treasury_stader_sol_account;
                self.state.treasury_stader_sol_account = treasury_stader_sol_account;
                Some(PubkeyValueChange {
                    old,
                    new: treasury_stader_sol_account,
                })
            } else {
                None
            };
        ...
        emit!(ChangeAuthorityEvent {
            ...
            treasury_stader_sol_account_change,
