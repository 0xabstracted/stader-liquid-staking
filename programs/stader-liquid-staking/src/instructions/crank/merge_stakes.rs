...
use crate::{error::StaderLiquidStakingError, state::stake_system::StakeSystem, State};
...
impl<'info> MergeStakes<'info> {
    pub fn process(
        ...
        let destination_delegation = if let Some(delegation) = self.destination_stake.delegation() {
            ...
            return err!(StaderLiquidStakingError::DestinationStakeMustBeDelegated)
                .map_err(|e| e.with_account_name("destination_stake"));
        };
        require_eq!(
            destination_delegation.deactivation_epoch,
            std::u64::MAX,
            StaderLiquidStakingError::DestinationStakeMustNotBeDeactivating
        );
        require_eq!(
            destination_stake_info.last_update_delegated_lamports,
            destination_delegation.stake,
            StaderLiquidStakingError::DestinationStakeMustBeUpdated
        );

        require_keys_eq!(
            destination_delegation.voter_pubkey,
            validator.validator_account,
            StaderLiquidStakingError::InvalidDestinationStakeDelegation
        );
        ...
        let source_delegation = if let Some(delegation) = self.source_stake.delegation() {
            ...
            return err!(StaderLiquidStakingError::SourceStakeMustBeDelegated)
                .map_err(|e| e.with_account_name("source_stake"));
        };
        require_eq!(
            source_delegation.deactivation_epoch,
            std::u64::MAX,
            StaderLiquidStakingError::SourceStakeMustNotBeDeactivating
        );
        require_eq!(
            source_stake_info.last_update_delegated_lamports,
            source_delegation.stake,
            StaderLiquidStakingError::SourceStakeMustBeUpdated
        );
        ...
        require_keys_eq!(
            ...
            validator.validator_account,
            StaderLiquidStakingError::InvalidSourceStakeDelegation
        );
        invoke_signed(
