...
#[derive(Debug)]
pub struct State {
    pub stader_sol_mint: Pubkey,

    pub admin_authority: Pubkey,

    // Target for withdrawing rent reserve SOLs. Save bot wallet account here
    pub operational_sol_account: Pubkey,
    // treasury - external accounts managed by something like stader DAO
    // pub treasury_sol_account: Pubkey,
    pub treasury_stader_sol_account: Pubkey,

    // Bump seeds:
    pub reserve_bump_seed: u8,
    pub stader_sol_mint_authority_bump_seed: u8,
    ...
    pub stader_sol_supply: u64, // Virtual value (may be < because of token burn). Use Update* to align
    // For FE. Don't use it for token amount calculation
    pub stader_sol_price: u64,
...
impl State {
    ...
    pub const STADER_SOL_MINT_AUTHORITY_SEED: &'static [u8] = b"st_mint";
    ...
    pub fn find_stader_sol_mint_authority(state: &Pubkey) -> (Pubkey, u8) {
        Pubkey::find_program_address(
            &[&state.to_bytes()[..32], State::STADER_SOL_MINT_AUTHORITY_SEED],
            &ID,
        )
    }
    ...
    pub fn get_treasury_stader_sol_balance<'info>(
        &self,
        treasury_stader_sol_account: &AccountInfo<'info>,
    ) -> Option<u64> {
        if treasury_stader_sol_account.owner != &spl_token::ID {
            msg!(
                "treasury_stader_sol_account {} is not a token account",
                treasury_stader_sol_account.key
            );
            return None; // Not an error. Admins may decide to reject fee transfers to themselves
        }

        match spl_token::state::Account::unpack(treasury_stader_sol_account.data.borrow().as_ref()) {
            Ok(token_account) => {
                if token_account.mint == self.stader_sol_mint {
                    Some(token_account.amount)
                } else {
                    msg!(
                        "treasury_stader_sol_account {} has wrong mint {}. Expected {}",
                        treasury_stader_sol_account.key,
                        token_account.mint,
                        self.stader_sol_mint
                    );
            ...
            Err(e) => {
                msg!(
                    "treasury_stader_sol_account {} can not be parsed as token account ({})",
                    treasury_stader_sol_account.key,
                    e
                );
                None // Not an error. Admins may decide to reject fee transfers to themselves
    ...
    pub fn check_staking_cap(&self, transfering_lamports: u64) -> Result<()> {
        ...
        require_lte!(
            ...
            StaderLiquidStakingError::StakingIsCapped
        );
        Ok(())
    }
    ...
    /// calculate the amount of staderSOL tokens corresponding to certain lamport amount
    pub fn calc_stader_sol_from_lamports(&self, stake_lamports: u64) -> Result<u64> {
        shares_from_value(
            stake_lamports,
            self.total_virtual_staked_lamports(),
            self.stader_sol_supply,
        )
    }
    /// calculate lamports value from some stader_sol_amount
    /// result_lamports = stader_sol_amount * stader_sol_price
    pub fn stader_sol_to_sol(&self, stader_sol_amount: u64) -> Result<u64> {
        value_from_shares(
            stader_sol_amount,
            self.total_virtual_staked_lamports(),
            self.stader_sol_supply,
    ...
    pub fn on_stader_sol_mint(&mut self, amount: u64) {
        self.stader_sol_supply += amount
    }

    pub fn on_stader_sol_burn(&mut self, amount: u64) {
        self.stader_sol_supply -= amount
    }
    ...
    pub fn on_stake_moved(&mut self, amount: u64, clock: &Clock) -> Result<()> {
        ...
        require_lte!(
            ...
            StaderLiquidStakingError::MovingStakeIsCapped
        );
        Ok(())
    }
}
