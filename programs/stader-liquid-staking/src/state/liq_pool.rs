use crate::{calc::proportional, error::StaderLiquidStakingError, require_lte, state::Fee, ID};
...
pub struct LiqPool {
    ...
    pub stader_sol_leg_authority_bump_seed: u8,
    pub stader_sol_leg: Pubkey,

    //The next 3 values define the SOL/staderSOL Liquidity pool fee curve params
    // We assume this pool is always UNBALANCED, there should be more SOL than staderSOL 99% of the time
    ...
    /// SOL/staderSOL Liquidity pool min fee
    pub lp_min_fee: Fee, //0.3% initially
    /// Treasury cut
    pub treasury_cut: Fee, //2500 => 25% how much of the Liquid unstake fee goes to treasury_stader_sol_account
...
impl LiqPool {
    ...
    pub const STADER_SOL_LEG_AUTHORITY_SEED: &'static [u8] = b"liq_st_sol_authority";
    pub const STADER_SOL_LEG_SEED: &'static str = "liq_st_sol";
    ...
    pub fn find_stader_sol_leg_authority(state: &Pubkey) -> (Pubkey, u8) {
        Pubkey::find_program_address(
            &[&state.to_bytes()[..32], Self::STADER_SOL_LEG_AUTHORITY_SEED],
            &ID,
        )
    }

    pub fn default_stader_sol_leg_address(state: &Pubkey) -> Pubkey {
        Pubkey::create_with_seed(state, Self::STADER_SOL_LEG_SEED, &spl_token::ID).unwrap()
    }
    ...
    pub fn check_liquidity_cap(
        ...
        require_lte!(
            ...
            StaderLiquidStakingError::LiquidityIsCapped
        );
        Ok(())
    }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        require_lte!(
            self.lp_max_fee,
            Self::MAX_FEE,
            StaderLiquidStakingError::LpMaxFeeIsTooHigh
        );
        require_gte!(
            self.lp_max_fee,
            self.lp_min_fee,
            StaderLiquidStakingError::LpFeesAreWrongWayRound
        );
        require_gte!(
            self.lp_liquidity_target,
            Self::MIN_LIQUIDITY_TARGET,
            StaderLiquidStakingError::LiquidityTargetTooLow
        );
        require_lte!(
            self.treasury_cut,
            Self::MAX_TREASURY_CUT,
            StaderLiquidStakingError::TreasuryCutIsTooHigh
        );
